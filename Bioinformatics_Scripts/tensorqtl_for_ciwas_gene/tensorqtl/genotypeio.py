...
class InputGeneratorCis(object):
    ...
    """
    def __init__(self, tad_df, variant_df, phenotype_df, phenotype_pos_df, group_s=None, window=1000000):
        assert (tad_df.index == variant_df.index).all()
        assert (phenotype_df.index == phenotype_df.index.unique()).all()
        self.genotype_df = tad_df
        ...
        if any(~m):
            print(f'    ** dropping {sum(~m)} phenotypes on chrs. without TADs')
        ...
        self.cis_ranges, drop_ids = get_cis_ranges_tad(self.phenotype_pos_df, self.chr_variant_dfs, self.window)

        if len(drop_ids) > 0:
            print(f"    ** dropping {len(drop_ids)} phenotypes without TADs in cis-window")
...
def load_tad_data(tad_file_path, select_samples=None):
    """读取TAD IS分数数据文件"""
    tad_df = pd.read_csv(tad_file_path, sep='\t', comment=None)
    tad_df.columns = [col.lstrip('#') for col in tad_df.columns]
    
    # 检查gid唯一性
    if tad_df['gid'].duplicated().any():
        raise ValueError("TAD文件中存在重复的gid")
    
    # 位置信息
    tad_variant_df = tad_df[['chr', 'end']].copy()
    tad_variant_df.columns = ['chrom', 'pos'] 
    tad_variant_df.index = tad_df['gid']
    
    # IS分数矩阵
    sample_cols = tad_df.columns[6:]
    tad_scores_df = tad_df[sample_cols].copy()
    tad_scores_df.index = tad_df['gid']
    tad_scores_df = tad_scores_df.astype(float)  # 确保浮点数类型
    
    # 样本筛选
    if select_samples is not None:
        missing_samples = set(select_samples) - set(sample_cols)
        if missing_samples:
            raise ValueError(f"Missing samples in TAD file: {missing_samples}")
        tad_scores_df = tad_scores_df[select_samples]
    
    # 缺失值检查
    if tad_scores_df.isnull().any().any():
        print("Warning: TAD文件中存在缺失值")
    
    return tad_scores_df, tad_variant_df


def get_cis_ranges_tad(phenotype_pos_df, chr_variant_dfs, window, verbose=True):
    """
    计算基因TSS±window范围内的TAD边界
    
    phenotype_pos_df: 基因位置信息（TSS位置）
    chr_variant_dfs: TAD位置信息（TAD边界位置）
    """
    # 复制避免修改原始数据
    ...
    for k, phenotype_id in enumerate(phenotype_pos_df.index, 1):
        ...
        if chrom not in chr_variant_dfs:
            drop_ids.append(phenotype_id)
            continue
            
        # 基因TSS位置
        gene_tss = pos['start']  # 或pos['end']，取决于BED文件格式
        
        # 计算窗口范围
        window_start = gene_tss - window  
        window_end = gene_tss + window
        
        # 找到落在窗口内的TAD边界（使用TAD的end位置）
        tad_positions = chr_variant_dfs[chrom]['pos'].values
        tad_indices = chr_variant_dfs[chrom]['index'].values
        
        # 找到窗口内的TAD（闭区间）
        mask = (tad_positions >= window_start) & (tad_positions <= window_end)
        valid_indices = tad_indices[mask]
        
        if len(valid_indices) > 0:
            cis_ranges[phenotype_id] = [valid_indices.min(), valid_indices.max()]
        else:
            drop_ids.append(phenotype_id)
...
def validate_sample_consistency(phenotype_df, tad_df, covariates_df=None):
    """验证表型、TAD、协变量数据的样本一致性"""
    phenotype_samples = set(phenotype_df.columns)
    tad_samples = set(tad_df.columns)
    
    if phenotype_samples != tad_samples:
        missing_in_tad = phenotype_samples - tad_samples
        missing_in_phenotype = tad_samples - phenotype_samples
        
        error_msg = []
        if missing_in_tad:
            error_msg.append(f"TAD文件缺少样本: {missing_in_tad}")
        if missing_in_phenotype:
            error_msg.append(f"表型文件缺少样本: {missing_in_phenotype}")
        
        raise ValueError("\n".join(error_msg))
    
    if covariates_df is not None:
        covariate_samples = set(covariates_df.index)
        if phenotype_samples != covariate_samples:
            raise ValueError("协变量文件样本与表型文件不匹配")
    
    # 确保样本顺序一致（创建副本避免修改原数据）
    sample_order = phenotype_df.columns.tolist()
    tad_df = tad_df[sample_order].copy()  # 添加copy()
    if covariates_df is not None:
        covariates_df = covariates_df.loc[sample_order].copy()  # 添加copy()
    
    return tad_df, covariates_df
